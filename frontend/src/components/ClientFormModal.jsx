...
export default function ClientFormModal({ projetoParaEditar, clientesExistentes = [], aoSalvarProjeto, aoCriarClienteNovo, aoFechar }) {
  ...
  // ESTADO NOVO: Para controlar o aviso visual do CEP
  ...
  const buscarCep = async (cepValor) => {
    ...
    if (cepLimpo.length === 8) {
      setBuscandoCep(true); // Ativa o aviso de carregamento
      try {
        ...
        setBuscandoCep(false); // Desliga o aviso quando termina (dando certo ou errado)
      }
    }
  };
    ...
    <div className="modal-overlay">
      <div className="modal-content" style={{ width: '650px', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* ENDEREÇO DA OBRA COM AVISO DE CEP */}
          <div style={{ display: 'flex', gap: '10px', marginBottom: '8px' }}>
            <div style={{ width: '120px', position: 'relative' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', color: '#5e6c84', marginBottom: '5px' }}>
                CEP
              </label>
            ...
            <div style={{ flex: 2 }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', color: '#5e6c84', marginBottom: '5px' }}>
                Rua {buscandoCep && <span style={{ color: '#0052cc', fontWeight: 'normal', fontStyle: 'italic' }}>Buscando... ⏳</span>}
              </label>
          ...
          {/* Selo verde que aparece quando o CEP preenche a rua com sucesso */}
          {formProjeto.rua && !buscandoCep && (
            <div style={{ fontSize: '11px', color: '#15803D', marginBottom: '12px', marginLeft: '130px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
            {/* type="button" garante que ele não vai tentar enviar o form */}
